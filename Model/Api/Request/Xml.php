...
class Xml extends Request
{
     ...
     * @return string
     */
    private function getDataAsXml()
    {
        $xml = new XMLExtended("<request/>");
        $this->convertArrayToXml($this->getData(), $xml);
     ...
     */
    private function convertArrayToXml(array $array, XMLExtended &$parent): void
    {
        $flag = 0;
        foreach ($array as $key => $value) {
            if (is_array($value)) {
                if (is_numeric($key)) {
                    $this->convertArrayToXml($value, $parent);
                } else {
                    $child = $parent->addChild($key);
                    $this->convertArrayToXml($value, $child);
                }
            } else {
                $key = (is_numeric($key))
                    ? "item" . $key
     ...
     * @return string
     */
    private function stripInvalidXml(string $value)
    {
        $ret = "";
